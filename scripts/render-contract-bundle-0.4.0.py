#!/usr/bin/env python3
"""Render the authoritative substrate-wire 0.4.0 development bundle.
...
BUNDLE = ROOT / "contracts" / "substrate-wire" / "0.4.0"
...
MAX_CAPSULE_FILES = 32
MAX_CAPSULE_FILE_BYTES = 262_144
MAX_ENCODED_CAPSULE_FILE_BYTES = 349_528
MAX_CAPSULE_BYTES = 524_288
...
COMMON = schema(
    ...
    **{
        "$defs": {
            ...
            "session-id": {"pattern": "^ses_[A-Za-z0-9]+$", "type": "string"},
            ...
            "facts": closed_object(
                {
                    ...
                    "exec.inline-capsule": closed_object(
                        {
                            "mount": {"const": "/runtime"},
                            "max_files": {"const": MAX_CAPSULE_FILES},
                            "max_file_bytes": {"const": MAX_CAPSULE_FILE_BYTES},
                            "max_total_bytes": {"const": MAX_CAPSULE_BYTES},
                        },
                        ["mount", "max_files", "max_file_bytes", "max_total_bytes"],
                    ),
...
APPLIED_CAPSULE = closed_object(
    {
        "entrypoint": {"pattern": "^/runtime/(?!.*(?:^|/)\\.\\.(?:/|$))[^\\u0000]+$", "type": "string"},
        "file_count": {"maximum": MAX_CAPSULE_FILES, "minimum": 1, "type": "integer"},
        "manifest_sha256": {"pattern": "^[0-9a-f]{64}$", "type": "string"},
        "mount": {"const": "/runtime"},
        "total_bytes": {"maximum": MAX_CAPSULE_BYTES, "minimum": 0, "type": "integer"},
    },
    ["manifest_sha256", "entrypoint", "mount", "file_count", "total_bytes"],
)
        ...
        closed_object(
            {
                ...
                "cgroup": {"maxLength": 512, "minLength": 1, "type": "string"},
                "filesystem": {"const": "workspace-rw-capsule-ro-system-ro"},
                "network": {"const": "none"},
                "profile": {"const": "workspace"},
            },
            ["capability_snapshot", "profile", "filesystem", "network", "cgroup", "capsule"],
        ),
    ]
}
...
PIPE_SESSION_LIMITS = closed_object(
    {
        "frame_bytes": {"maximum": 65536, "minimum": 1, "type": "integer"},
        "input_bytes": {"maximum": 16777216, "minimum": 1, "type": "integer"},
        "queued_frames": {"maximum": 16, "minimum": 1, "type": "integer"},
    },
    ["input_bytes", "frame_bytes", "queued_frames"],
)

PIPE_SESSION_BASE = {
    "attachment": {"enum": ["pending", "available", "attached", "consumed", "uncertain"]},
    "capability_snapshot": {"pattern": "^sha256:[0-9a-f]{64}$", "type": "string"},
    "exec": {"$ref": "common.json#/$defs/exec-id"},
    "exit": {"oneOf": [EXIT, {"type": "null"}]},
    "id": {"$ref": "common.json#/$defs/session-id"},
    "kind": {"const": "session"},
    "lease": LEASE,
    "limits": PIPE_SESSION_LIMITS,
    "mode": {"const": "pipes"},
    "observed_at": {"$ref": "common.json#/$defs/timestamp"},
    "state": {"enum": ["accepted", "ready", "attached", "exited", "cancelled", "expired", "unknown"]},
    "workspace": {"$ref": "common.json#/$defs/workspace-id"},
}
PIPE_SESSION_REQUIRED = [
    "id", "kind", "mode", "exec", "workspace", "state", "attachment", "observed_at",
    "capability_snapshot", "limits", "exit", "lease",
]
PIPE_SESSION = {
    "oneOf": [
        closed_object({**PIPE_SESSION_BASE, "attachment": {"const": "pending"}, "exit": {"type": "null"}, "state": {"const": "accepted"}}, PIPE_SESSION_REQUIRED),
        closed_object({**PIPE_SESSION_BASE, "attachment": {"const": "available"}, "exit": {"type": "null"}, "state": {"const": "ready"}}, PIPE_SESSION_REQUIRED),
        closed_object({**PIPE_SESSION_BASE, "attachment": {"const": "attached"}, "exit": {"type": "null"}, "state": {"const": "attached"}}, PIPE_SESSION_REQUIRED),
        closed_object({**PIPE_SESSION_BASE, "attachment": {"const": "consumed"}, "exit": EXIT, "state": {"enum": ["exited", "cancelled"]}}, PIPE_SESSION_REQUIRED),
        closed_object({**PIPE_SESSION_BASE, "attachment": {"const": "consumed"}, "state": {"const": "expired"}}, PIPE_SESSION_REQUIRED),
        closed_object({**PIPE_SESSION_BASE, "attachment": {"const": "uncertain"}, "state": {"const": "unknown"}}, PIPE_SESSION_REQUIRED),
    ]
}
...
RESOURCE = schema(
    ...
    **{
        "$defs": {
            ...
            "workspace": WORKSPACE,
            "exec": EXEC,
            "pipe-session": PIPE_SESSION,
            "lease": LEASE,
        },
        "oneOf": [
            {"$ref": "#/$defs/workspace"},
            {"$ref": "#/$defs/exec"},
            {"$ref": "#/$defs/pipe-session"},
        ],
    },
)
...
EXECUTION_CAPSULE_FILE = closed_object(
    {
        "content": closed_object(
            {
                "data": {"$ref": "../common.json#/$defs/canonical-base64-capsule-file"},
                "encoding": {"const": "base64"},
            },
            ["encoding", "data"],
        ),
        ...
        "role": {"enum": ["runtime", "configuration", "hook", "protocol-sidecar"]},
        "sha256": {"pattern": "^[0-9a-f]{64}$", "type": "string"},
    },
    ["path", "role", "executable", "sha256", "content"],
)

EXECUTION_CAPSULE = closed_object(
    {
        "entrypoint": {"$ref": "../common.json#/$defs/relative-path"},
        "files": {
            "items": EXECUTION_CAPSULE_FILE,
...
pipe_session_start_input = add_input(
    "pipe-session-start",
    closed_object(
        {
            "exec": closed_object(
                {
                    **EXEC_START_INPUT["properties"],
                    "lease_ttl_ms": {"maximum": 86400000, "minimum": 1000, "type": "integer"},
                    "wait": {"const": False},
                },
                ["workspace", "argv", "env", "sandbox", "limits", "wait", "lease_ttl_ms"],
            ),
            "frame_limit_bytes": {"maximum": 65536, "minimum": 1, "type": "integer"},
            "input_limit_bytes": {"maximum": 16777216, "minimum": 1, "type": "integer"},
            "queued_frames": {"maximum": 16, "minimum": 1, "type": "integer"},
        },
        ["exec", "input_limit_bytes", "frame_limit_bytes", "queued_frames"],
    ),
)
pipe_session_get_input = add_input("pipe-session-get", EMPTY)
pipe_session_attach_input = add_input("pipe-session-attach", EMPTY)
pipe_session_retire_input = add_input("pipe-session-retire", EMPTY)
...
ADDRESS_BY_OPERATION = {
    ...
    "session.capabilities": add_address("pipe-session-capabilities", EMPTY),
    "session.start": add_address("pipe-session-start", EMPTY),
    "session.get": add_address(
        "pipe-session-get",
        closed_object({"session_id": {"$ref": "../common.json#/$defs/session-id"}}, ["session_id"]),
    ),
    "session.attach": add_address(
        "pipe-session-attach",
        closed_object({"session_id": {"$ref": "../common.json#/$defs/session-id"}}, ["session_id"]),
    ),
    "session.signal": add_address(
        "pipe-session-signal",
        closed_object({"session_id": {"$ref": "../common.json#/$defs/session-id"}}, ["session_id"]),
    ),
    "session.retire": add_address(
        "pipe-session-retire",
        closed_object({"session_id": {"$ref": "../common.json#/$defs/session-id"}}, ["session_id"]),
    ),
    "session.lease.renew": add_address(
        "pipe-session-lease-renew",
        closed_object({"session_id": {"$ref": "../common.json#/$defs/session-id"}}, ["session_id"]),
    ),
...
pipe_session_result = {"$ref": "../resource.json#/$defs/pipe-session"}
pipe_session_start_result = add_result("pipe-session-start", pipe_session_result)
pipe_session_get_result = add_result("pipe-session-get", pipe_session_result)
pipe_session_signal_result = add_result("pipe-session-signal", pipe_session_result)
pipe_session_lease_renew_result = add_result("pipe-session-lease-renew", pipe_session_result)
pipe_session_retire_result = add_result(
    "pipe-session-retire", ABSENCE("session", "../common.json#/$defs/session-id")
)
pipe_session_capabilities_result = add_result(
    "pipe-session-capabilities",
    closed_object(
        {
            "capability_snapshot": {"pattern": "^sha256:[0-9a-f]{64}$", "type": "string"},
            "contract": {"const": "substrate-wire/0.4.0"},
            "lease_required": {"const": True},
            "max_frame_bytes": {"const": 65536},
            "max_input_bytes": {"const": 16777216},
            "max_queued_frames": {"const": 16},
            "network": {"const": "none"},
            "single_attachment": {"const": True},
            "transport": {"const": "unix-websocket-json"},
        },
        [
            "contract", "transport", "capability_snapshot", "lease_required",
            "single_attachment", "network", "max_input_bytes", "max_frame_bytes",
            "max_queued_frames",
        ],
    ),
)
pipe_session_attach_result = add_result(
    "pipe-session-attach",
    closed_object(
        {
...
ROUTES = [
    ...
    ("session.capabilities", "GET", "/v1/pipe-sessions", "observe", "read", "idempotent", [], "callable", machine_input, pipe_session_capabilities_result, "query", [predicate("exec.namespaces", "eq", value={"user": True, "mount": True, "pid": True, "ipc": True, "uts": True, "network": True}), predicate("exec.cgroup-kill", "eq", value=True), predicate("exec.no-egress", "eq", value=True)]),
    ("session.start", "POST", "/v1/pipe-sessions", "session", "write", "keyed", ["process", "filesystem:workspace"], "projected", pipe_session_start_input, pipe_session_start_result, "body.input", [predicate("exec.namespaces", "eq", value={"user": True, "mount": True, "pid": True, "ipc": True, "uts": True, "network": True}), predicate("exec.cgroup-limits", "eq", value={"processes": True, "memory": True, "cpu": True}), predicate("exec.cgroup-kill", "eq", value=True), predicate("exec.no-egress", "eq", value=True), predicate("leases.explicit", "eq", value=True)]),
    ("session.get", "GET", "/v1/pipe-sessions/{session_id}", "session", "read", "idempotent", [], "callable", pipe_session_get_input, pipe_session_get_result, "query", []),
    ("session.attach", "GET", "/v1/pipe-sessions/{session_id}/attach", "session", "write", "none", ["process"], "callable-direct", pipe_session_attach_input, pipe_session_attach_result, "query", [predicate("exec.cgroup-kill", "eq", value=True)]),
    ("session.signal", "POST", "/v1/pipe-sessions/{session_id}/signal", "session", "destructive", "keyed", ["process"], "projected", exec_signal_input, pipe_session_signal_result, "body.input", [predicate("exec.cgroup-kill", "eq", value=True)]),
    ("session.retire", "DELETE", "/v1/pipe-sessions/{session_id}", "session", "destructive", "keyed", [], "callable", pipe_session_retire_input, pipe_session_retire_result, "body.input", []),
    ("session.lease.renew", "POST", "/v1/pipe-sessions/{session_id}/lease/renew", "session", "write", "keyed", [], "callable", lease_renew_input, pipe_session_lease_renew_result, "body.input", [predicate("leases.explicit", "eq", value=True)]),
                ...
                "items": closed_object(
                    {
                        ...
                        "required_scope": {"enum": ["observe", "workspaces", "exec", "session"]},
...
HASHING = {
    ...
    "capsule_manifest": {
        ...
        "fields": ["domain", "entrypoint", "files"],
        "length_unit": "octets",
    },
...
def pipe_session_resource(
    operation: str = "01JPHASE4SESSIONSTART001",
    state: str = "ready",
    attachment: str = "available",
) -> dict[str, object]:
    return {
        ...
        "lease": {
            "actor": "vector-client",
            "authorizing_operation": operation,
            "clock_tolerance_ms": 30000,
            "principal": None,
            "renew_by": "2026-08-13T12:01:00Z",
            "state": "active",
            "ttl_ms": 60000,
        },
        "limits": {"frame_bytes": 65536, "input_bytes": 1048576, "queued_frames": 16},
        "mode": "pipes",
...
pipe_start_op = "01JPHASE4SESSIONSTART001"
pipe_start_input = {
    "exec": {**start_input, "lease_ttl_ms": 60000},
    "frame_limit_bytes": 65536,
    "input_limit_bytes": 1048576,
    "queued_frames": 16,
}
add_vector("http", "pipe-session-start", http_vector(
    "confined-pipe-session-start-binds-durable-session-and-exec",
    [
        "route.session.start", "session.distinct-identity", "session.profile-binding",
        "security.daemon-environment", "security.no-egress",
    ],
    "session.start",
    mutation_request("POST", "/v1/pipe-sessions", pipe_start_op, pipe_start_input, "req_pipe_start001"),
    202,
    success_body("req_pipe_start001", pipe_session_resource(), pipe_start_op),
    setup=[fixture("machine", "minimum-host", MACHINE), fixture("workspace", "owned", {"owner": SUBJECT})],
    postconditions=[
        condition("/driver/dispatch_count", "equals", 1),
        condition("/probes/session_id_distinct_from_exec_id", "equals", True),
        condition("/probes/child_environment_keys", "equals", []),
        condition("/probes/applied_network", "equals", "none"),
    ],
    phase=4,
))

pipe_start_without_lease = {
    **pipe_start_input,
    "exec": {key: value for key, value in pipe_start_input["exec"].items() if key != "lease_ttl_ms"},
}
add_vector("http", "pipe-session-missing-lease", http_vector(
    "pipe-session-without-explicit-lease-is-refused-before-dispatch",
    ["route.session.start", "session.lease-authority", "schema.strict-request"],
    "session.start",
    mutation_request(
        "POST", "/v1/pipe-sessions", "01JPHASE4SESSIONNOLEASE1",
        pipe_start_without_lease, "req_pipe_nolease1",
    ),
    422,
    error_body(
        "req_pipe_nolease1", "refused", "request.schema-invalid",
        "Request does not match the closed operation schema.",
        operation="01JPHASE4SESSIONNOLEASE1", address="input",
    ),
    setup=[fixture("machine", "minimum-host", MACHINE), fixture("workspace", "owned", {"owner": SUBJECT})],
    postconditions=[condition("/driver/dispatch_count", "equals", 0)],
    valid_input=False,
    phase=4,
))
...
for filename, vector_id, coverage, command, expected, postconditions in [
    (
        "pipe-session-capabilities", "pipe-session-capabilities-are-confinement-gated",
        ["route.session.capabilities"], {"operation": "session.capabilities"},
        {"contract": "substrate-wire/0.4.0", "network": "none", "single_attachment": True},
        [condition("/probes/delegated_confinement", "equals", True)],
    ),
    (
        "pipe-session-single-attachment", "pipe-session-attachment-right-is-consumed-once",
        ["route.session.get", "route.session.attach", "session.single-attachment"],
        {"attempts": 2, "operation": "session.attach", "session": "ses_vector"},
        {"first": "attached", "second": "conflict"},
        [condition("/probes/active_attachment_count", "equals", 1)],
    ),
    (
        "pipe-session-attachment-loss", "attachment-loss-kills-the-whole-process-tree",
        ["route.session.signal", "session.attachment-loss-containment", "security.process-tree-cancellation"],
        {"operation": "session.detach", "reason": "transport-loss", "session": "ses_vector"},
        {"attachment": "consumed", "state": "cancelled"},
        [condition("/probes/cgroup_process_count", "equals", 0)],
    ),
    (
        "pipe-session-protocol-failure", "invalid-pipe-frame-kills-the-whole-process-tree",
        ["session.protocol-failure-containment"],
        {"frame": {"kind": "close-input", "sequence": 2}, "operation": "session.frame"},
        {"code": "session.sequence-invalid", "state": "cancelled"},
        [condition("/probes/cgroup_process_count", "equals", 0)],
    ),
    (
        "pipe-session-restart", "restart-never-redispatches-or-reattaches-a-pipe-session",
        ["session.restart-no-redispatch"],
        {"operation": "daemon.restart", "session": "ses_vector"},
        {"attachment": "uncertain", "state": "unknown"},
        [condition("/driver/repeated_mutation_count", "equals", 0)],
    ),
    (
        "pipe-session-terminal-evidence", "pipe-terminal-exec-and-session-evidence-commit-together",
        ["session.exact-terminal-evidence"],
        {"operation": "session.observe-terminal", "session": "ses_vector"},
        {"attachment": "consumed", "exec_state": "exited", "session_state": "exited"},
        [condition("/probes/projection_mismatch", "equals", False)],
    ),
    (
        "pipe-session-retire", "session-retirement-removes-session-and-private-exec-atomically",
        ["route.session.retire"],
        {"operation": "session.retire", "session": "ses_vector"},
        {"exec_absent": True, "session_absent": True},
        [condition("/probes/orphaned_exec_row", "equals", False)],
    ),
    (
        "pipe-session-lease-renew", "session-renewal-updates-the-single-exec-lease-authority",
        ["route.session.lease.renew"],
        {"operation": "session.lease.renew", "session": "ses_vector", "ttl_ms": 60000},
        {"lease_rows": 1, "state": "active"},
        [condition("/probes/competing_lease_rows", "equals", 0)],
    ),
]:
    add_vector("driver", filename, driver_vector(
        vector_id, coverage, "lifecycle", command, expected,
        setup=[fixture("session", "durable-pipe", {"exec": "ex_vector", "id": "ses_vector"})],
        postconditions=postconditions, phase=4,
    ))

for filename, vector_id, requirement, command, expected, postconditions in [
    (
        "execution-capsule-digest-drift",
        "capsule-content-drift-refuses-before-dispatch",
        "security.capsule-digest-binding",
        {"operation": "capsule.materialize", "mutation": "replace-file-bytes"},
        {"code": "capsule.invalid", "outcome": "not-dispatched"},
        [condition("/driver/dispatch_count", "equals", 0)],
    ),
    (
        "execution-capsule-path-escape",
        "capsule-path-escape-refuses-before-materialization",
        "security.capsule-path-closure",
        {"operation": "capsule.materialize", "path": "../escape"},
        {"code": "capsule.invalid", "outcome": "not-dispatched"},
        [condition("/probes/outside_file_exists", "equals", False)],
    ),
    (
        "execution-capsule-read-only",
        "capsule-files-are-read-only-inside-the-sandbox",
        "security.capsule-read-only",
        {"operation": "capsule.exec", "write": "/runtime/config/effective.json"},
        {"write_succeeded": False},
        [condition("/probes/capsule_sha256_changed", "equals", False)],
    ),
    (
        "execution-capsule-workspace-separation",
        "capsule-and-workspace-have-distinct-write-postures",
        "security.capsule-workspace-separation",
        {"operation": "capsule.exec", "workspace": "ws_vector"},
        {"capsule_mount": "read-only", "workspace_mount": "read-write"},
        [condition("/probes/runtime_payload_in_workspace", "equals", False)],
    ),
    (
        "execution-capsule-cleanup",
        "capsule-directory-is-removed-after-tree-reconciliation",
        "security.capsule-cleanup",
        {"operation": "capsule.reconcile", "exec": "ex_vector"},
        {"capsule_absent": True, "exec_state": "exited"},
        [condition("/probes/cgroup_process_count", "equals", 0)],
    ),
]:
    add_vector("driver", filename, driver_vector(
        vector_id, [requirement], "security", command, expected,
        setup=[fixture("capsule", "bounded-inline", {"mount": "/runtime"})],
        postconditions=postconditions, phase=4,
    ))

add_vector("driver", "execution-capsule-restart-cleanup", driver_vector(
    "restart-removes-capsule-only-after-orphan-tree-reconciliation",
    ["security.capsule-restart-reconciliation"],
    "security",
    {"operation": "daemon.restart", "exec": "ex_vector"},
    {"capsule_absent": True, "exec_state": "unknown", "redispatched": False},
    setup=[
        fixture("capsule", "bounded-inline", {"mount": "/runtime"}),
        fixture("crash", "after-capsule-materialization", {"exec": "ex_vector"}),
    ],
    postconditions=[
        condition("/probes/cgroup_process_count", "equals", 0),
        condition("/driver/repeated_mutation_count", "equals", 0),
...
PIPE_CONTENT = closed_object(
    {
        "data": {"maxLength": 87384, "pattern": CANONICAL_BASE64_PATTERN, "type": "string"},
        "encoding": {"const": "base64"},
    },
    ["encoding", "data"],
)
PIPE_CHANNEL_FRAME = schema(
    "Closed bounded raw-pipe WebSocket frame vocabulary",
    schema_id="pipe-channel-frame",
    **{
        "oneOf": [
            closed_object({"content": PIPE_CONTENT, "kind": {"const": "stdin"}, "sequence": {"maximum": U64_MAX, "minimum": 1, "type": "integer"}}, ["kind", "sequence", "content"]),
            closed_object({"kind": {"const": "close-input"}, "sequence": {"maximum": U64_MAX, "minimum": 1, "type": "integer"}}, ["kind", "sequence"]),
            closed_object({"grace_ms": {"maximum": 60000, "minimum": 0, "type": "integer"}, "kind": {"const": "signal"}, "sequence": {"maximum": U64_MAX, "minimum": 1, "type": "integer"}, "signal": {"enum": ["INT", "TERM", "KILL"]}}, ["kind", "sequence", "signal", "grace_ms"]),
            closed_object({"content": PIPE_CONTENT, "kind": {"const": "output"}, "sequence": {"maximum": U64_MAX, "minimum": 1, "type": "integer"}, "stream": {"enum": ["stdout", "stderr"]}}, ["kind", "sequence", "stream", "content"]),
            closed_object({"kind": {"const": "truncated"}, "sequence": {"maximum": U64_MAX, "minimum": 1, "type": "integer"}, "stream": {"enum": ["stdout", "stderr"]}}, ["kind", "sequence", "stream"]),
            closed_object({"exit": {"oneOf": [EXIT, {"type": "null"}]}, "kind": {"const": "exit"}, "sequence": {"maximum": U64_MAX, "minimum": 1, "type": "integer"}, "state": {"enum": ["exited", "cancelled", "expired", "unknown"]}}, ["kind", "sequence", "state", "exit"]),
            closed_object({"code": {"pattern": "^session\\.[a-z0-9-]+$", "type": "string"}, "kind": {"const": "protocol-error"}, "message": {"maxLength": 1024, "minLength": 1, "type": "string"}, "sequence": {"maximum": U64_MAX, "minimum": 1, "type": "integer"}}, ["kind", "sequence", "code", "message"]),
        ]
    },
)
            ...
            "generator": closed_object(
                {
                    ...
                    "name": {"const": "scripts/render-contract-bundle-0.4.0.py"},
                    "version": {"const": "1"},
                },
                ["name", "version", "digest"],
            ),
...
COMPATIBILITY_SCHEMA = schema(
    "Development compatibility declaration",
    ...
    **closed_object(
        ...
        ["$schema", "contract", "version", "status", "supported_api_majors", "request_policy", "response_policy", "development_constraints"],
    ),
)
...
HASHING_SCHEMA = schema(
    ...
    **closed_object(
            ...
            "capsule_manifest": closed_object(
                {
                    ...
                    "fields": {"const": ["domain", "entrypoint", "files"]},
                    "length_unit": {"const": "octets"},
                },
                ["domain", "encoding", "length_unit", "fields", "file_order", "file_fields"],
            ),
        ...
        ["$schema", "format", "algorithm", "canonical_input", "canonical_query", "capsule_manifest", "address_normalization", "tuple", "excluded", "ledger_key", "fixtures"],
    ),
)
...
def render() -> None:
    fixed = {
        ...
        "schemas/pipe-channel-frame.json": PIPE_CHANNEL_FRAME,
    ...
    manifest = {
        ...
        "generator": {
            ...
            "name": "scripts/render-contract-bundle-0.4.0.py",
        "version": "1",
        },
        "name": "substrate-wire",
        "origin": "b10x",
