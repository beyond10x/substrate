...
def check_manifest(
    ...
    renderer = ROOT / "scripts" / "render-contract-bundle-0.4.0.py"
    if not isinstance(generator, dict) or generator.get("name") != "scripts/render-contract-bundle-0.4.0.py":
        failures.append("bundle.json: generator must name scripts/render-contract-bundle-0.4.0.py")
...
def check_renderer_reproducibility(failures: list[str]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="substrate-contract-render-") as temporary:
        ...
        clean_bundle = clean_root / "contracts" / "substrate-wire" / "0.4.0"
...
def main() -> int:
    ...
    json_documents = check_json_authority(BUNDLE, "0.4.0", documents, validate, failures)
    ...
    if failures:
        ...
        return 1
    print(
        f"substrate-wire 0.4.0: {len(listed)} files, {json_documents} classified JSON documents, "
