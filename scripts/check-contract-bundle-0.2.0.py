...
def check_manifest(
    ...
    renderer = ROOT / "scripts" / "render-contract-bundle-0.2.0.py"
    if not isinstance(generator, dict) or generator.get("name") != "scripts/render-contract-bundle-0.2.0.py":
        failures.append("bundle.json: generator must name scripts/render-contract-bundle-0.2.0.py")
    ...
    if executable != sorted(RUNTIME_EXECUTED_VECTORS):
        failures.append(
            "bundle.json: executable_vectors must match the exact 0.2 vectors executed "
...
def check_renderer_reproducibility(failures: list[str]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="substrate-contract-render-") as temporary:
        ...
        clean_bundle = clean_root / "contracts" / "substrate-wire" / "0.2.0"
...
def check_registry(documents: Documents, failures: list[str]) -> tuple[dict[str, dict[str, object]], Path]:
    ...
    if observed != EXPECTED_ROUTES:
        failures.append(f"operations.json: route inventory differs from exact phase-3 set: {observed!r}")
...
def main() -> int:
    ...
    json_documents = check_json_authority(BUNDLE, "0.2.0", documents, validate, failures)
    ...
    if failures:
        ...
        return 1
    print(
        f"substrate-wire 0.2.0: {len(listed)} files, {json_documents} classified JSON documents, "
