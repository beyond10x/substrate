...
The stream is bounded, and a client is told when it hits a bound. A subject may hold 4 metrics
...
stream is cut after 1 hour, and a client frame larger than 1024 bytes ends it. Reconnect in each
case; nothing is lost, because the stream carries no history to lose.
