...
| Remote serving | agent-platform and other services address one Substrate instance over HTTPS/WSS with Identity-scoped authority | the promoted contract surface is selected; TLS and trust-envelope designs are accepted before code | remote SDK and clean-room conformance prove durable recovery, event gaps, session authority and negative TLS/auth cases | active — production TLS, hosted Identity admission and proof-bound session authority are implemented; remote SDK transport remains |
...
contract bundle or a promise that the current `substrate-wire/0.14.0` development bundle is stable.
