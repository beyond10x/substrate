...
before durable admission. Remote SDK transport, a node-bound Kubernetes serving profile,
...
| Wire contract | bundles 0.1.0–0.13.0 remain frozen and reproducible; 0.13.0 is the advertised development frontier at inner `bundle.json` digest `7ab665c0a50cd8521a28b2b4c2302b7d460a1978b105d956711dbe6702a843bf`, directly preserves 0.12.0's 33 routes and adds no route while declaring hosted Identity admission; the gate also retains the recorded 0.12.0 lineage bridge | notify and observe Harness compatibility, publish the successor only through a later eligible release, and do not describe it as stable |
...
| Rust SDK | current source verifies the promoted 0.13.0 name and inner digest before serving an operation; exposes every promoted route through typed guarded-file, PTY, metrics, snapshot, event and bounded-output APIs; preserves optional capability facts; accepts caller ids on every mutation; serializes SDK observations; and supervises an external or linked daemon as a separate child whose orderly shutdown proves its process tree and cgroup absent | publish the owner-released package chain manually from a fully gated annotated tag, then add the separately designed HTTPS/WSS transport |
...
- The Rust workspace has five approved runtime registry packages — `b10x-substrate-wire`,
  ...
  0.13.0 is the current development bundle and every earlier bundle
  directory is frozen; [`scripts/gate.sh`](scripts/gate.sh) runs the four Python bundle checkers
  plus `cargo xtask check-bundle` for 0.5.0 through 0.13.0 on every invocation, so a green
  gate is evidence that all thirteen still hold. 0.5.0 is the first bundle whose checker is a `cargo xtask` verb
...
cargo xtask check-bundle 0.13.0                     # the current Wire contract row
cargo test --workspace --locked -- --nocapture       # the clean-room case inventory
cargo xtask check-toolchain                          # the pinned channel
cargo xtask package-bundle 0.13.0 --out <dir>        # the current OCI manifest digest
```
