...
The phase-3 implementation has nineteen bundled development routes; the 0.3.0 successor development
bundle adds seven phase-4 pipe-session routes. All 39 phase-3 review findings have
...
| Wire contract | immutable 0.1.0 remains byte-clean; deterministic 0.2.0 remains reproducible; deterministic 0.3.0 has 26 closed operations, 21 executable vectors, 65 design vectors, 106 requirements, and a reproducible fixed point | package, sign, and digest-pin a stable release without changing development authority implicitly |
...
| Implementation | phase 3 complete; the phase-4 raw-pipe slice has distinct durable session identity, session-native lifecycle operations, one scoped Unix-WebSocket attachment, atomic terminal/restart projection, and a green delegated model-free Agent lane | retain the raw-pipe evidence while adding only separately gated PTY, authority, and release work |
...
- Current Rust tests prove provisional dispatch before host mutation, full first-terminal-wins
  ...
  minimum host limits.
- All three bundle trees classify every JSON document and meta-validate declared Draft 2020-12 schemas
...
copied-contract fixture pass. The deterministic 0.3.0 bundle and real delegated Agent compatibility
lane now also pass, including bounded queue pressure, attachment/protocol loss, lease expiry,
restart-to-unknown, and whole-tree cleanup. This is development cross-repository confinement
