...
| Rust SDK | current source verifies the promoted 0.15.0 name and inner digest before serving an operation; sends session requests only to `/v1/sessions`; exposes typed guarded-file, PTY, metrics, snapshot, event and bounded-output APIs; preserves optional capability facts; accepts caller ids on every mutation; serializes SDK observations; and supervises an external or linked daemon as a separate child whose orderly shutdown proves its process tree and cgroup absent | add HTTPS/WSS transport and the attachment-authority mint/proof flow; distribute the SDK from source rather than crates.io |
