...
without adding a route. This is development conformance, not stable publication: nothing has been
published, signed or digest-pinned yet.
...
| Release | [`Dockerfile`](Dockerfile) builds the daemon and `cargo xtask package-bundle <version> --out <dir>` emits a deterministic OCI image layout from a frozen bundle (0.4.0 → manifest `sha256:3758e80bc39f1eb03b15c69410608c9ef1d2ba8095c7e707c6988dbb5894ab00`); since 2026-08-30 [`.github/workflows/release.yml`](.github/workflows/release.yml) defines the daemon-image half — an annotated bare-version tag at a commit with a `success` `gate.yml` conclusion builds, pushes, keyless-signs and digest-pins it, and a pre-release tag publishes nothing — but it has **never run**, its `B10X_BOT_APP_ID` and `B10X_BOT_PRIVATE_KEY` secrets are unset, and **no** workflow publishes or signs the bundle layout | run the release workflow from a real tag and record its published digest (`story:signed-daemon-image`); publish and sign the 0.4.0 bundle layout (`story:contract-bundle-oci-artifact`) |
