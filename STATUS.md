...
files, atomic SIGHUP rotation and fail-closed pre-admission. Hosted trust-envelope admission, remote
SDK transport, a node-bound Kubernetes serving profile, Docker/Kubernetes drivers and a direct
...
| Wire contract | bundles 0.1.0–0.12.0 remain frozen and reproducible; 0.12.0 is the advertised development frontier at inner `bundle.json` digest `1c82595a186ef1fa830a10e45fc842a037bb6bb7c5aafdc74e417681790e6360`; its frozen compatibility block compares with 0.10.0 (`preserves_routes: 31`, `adds_routes: 2`), while an additional gate proves the complete 0.11.0 route declarations and quota/metrics behavior survive beside exact read-only/scoped workspace access | retain the recorded one-time lineage bridge, notify and observe Harness compatibility, and do not describe the promoted bundle as stable |
...
| Rust SDK | current source verifies the promoted 0.12.0 name and inner digest before serving an operation; exposes every promoted route through typed guarded-file, PTY, metrics, snapshot, event and bounded-output APIs; preserves optional capability facts; accepts caller ids on every mutation; serializes SDK observations; and supervises an external or linked daemon as a separate child whose orderly shutdown proves its process tree and cgroup absent | publish the owner-released package chain manually from a fully gated annotated tag, then add the separately designed HTTPS/WSS transport |
