...
| Rust SDK | `b10x-substrate-sdk` verifies the advertised 0.4.0 contract and exposes typed workspace, file, exec, raw-pipe, event and operation handles, including workspace access, network, mounts, scratch, secret slots, capsules and exact usage; managed mode remains a separate child daemon | publish the owner-released package chain manually from a fully gated annotated tag, then close the remaining parity against the separately promoted contract |
