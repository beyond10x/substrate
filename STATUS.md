...
and 0025 plus the `0.12.0` bundle, scoped workspace writes and broader SDK parity. Release run
`33460657507` reused the anonymously resolvable bundle digest but refused while reading its
development annotation from the registry descriptor, before either artifact was signed or a daemon
image or GitHub release was published. `0.3.1` therefore remains the latest verified daemon image.

| Area | State | Next proof |
|---|---|---|
| Source | the standalone public repository `beyond10x/substrate` (`git remote -v`), public under atlas ADR 0003 ([CHANGELOG.md](CHANGELOG.md), 0.2.1); the latest annotated tag is `0.4.2` at `0687551`, while `0.3.1` remains the latest verified daemon publication | keep the portable-document and credential invariants enforced — `cargo xtask check-links` and `cargo xtask check-secrets` are full-gate steps |
...
| Release | [`Dockerfile`](Dockerfile) builds the daemon and `cargo xtask package-bundle <version> --out <dir>` emits a deterministic OCI image layout from a frozen bundle; development bundle `ghcr.io/beyond10x/b10x-substrate-wire:0.12.0` is anonymously resolvable at `sha256:dd901e848c821aca7d55f7b8cf5ee893e1d99a1428b348e32e7ed1045a375319` but is not yet signed; daemon image `0.3.1` is published, anonymously pullable, keyless-signed and verified at `sha256:48068ba167487fa84fe77e0acf7f8e3f556060f4d220a074d9b9f60f9af71ebf`; release run `33460657507` stopped before signing, daemon publication or announcement | merge the manifest-annotation fix, recover existing tag `0.4.2` from protected `main`, then record both observed digests through the normal gated pull request |
...
| Wire contract | bundles 0.1.0–0.11.0 remain frozen and reproducible; 0.12.0 is the current development frontier and its frozen compatibility block compares with 0.10.0 (`preserves_routes: 31`, `adds_routes: 2`) while adding exact read-only/scoped workspace access plus its capability and applied observation | prove the complete 0.11.0 inventory is preserved before advertising 0.12.0; package, sign and digest-pin the development bundle without describing it as stable |
