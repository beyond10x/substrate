...
async fn main() -> anyhow::Result<()> {
    ...
    let _instance_lock = lock_instance(&arguments.socket)?;
    ...
    if arguments.event_retention == 0 {
        bail!("event retention must be nonzero");
    }
    let store = Arc::new(
        Store::open_with_event_retention(&arguments.state, arguments.event_retention)
    ...
    host_config.event_retention = arguments.event_retention;
    ...
    let socket_cleanup = SocketCleanup(arguments.socket.clone());
    ...
    info!(socket = %arguments.socket.display(), "substrate ready");
